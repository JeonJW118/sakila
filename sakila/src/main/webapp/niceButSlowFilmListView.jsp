...
<body>
	...
	%>
			<a class="btn bg-dark text-white" href="<%=request.getContextPath()%>/niceButSlowerFilmist(view).jsp?currentPage=<%=currentPage-1%>">이전</a>&nbsp;&nbsp;&nbsp;
	<%	
		}
		if(currentPage < lastPage) { // 마지막페이지가 있다면 
	%>
		<a class="btn bg-dark text-white" href="<%=request.getContextPath()%>/niceButSlowerFilmList(view).jsp?currentPage=<%=currentPage+1%>">다음</a>&nbsp;&nbsp;&nbsp;
	<%		
		}
	%>
	</div>
